...
class Matcher:
    ...
    def match_one(self, pattern_set, maybe_tokenized):
        ...
        parsed_match = self._parse_match(best_match, self._doc)
    ...
    def match_many(self, pattern_set, maybe_tokenized):
        ...
        parsed_matches = [self._parse_match(m, self._doc) for m in all_matches]
    ...
    def _run_matcher(self, tokenized):
        logger.debug(f"Running the internal matcher on tokenized '{tokenized}'")
