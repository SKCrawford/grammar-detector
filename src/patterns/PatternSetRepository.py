...
class PatternSetRepository(Repository):
    def __init__(self):
        logger.info("Constructing the PatternSetRepository")
        # Callable[[T], str]
    ...
    def create(self, name: Name, data: PatternSetData) -> PatternSet:
        ...
        if self.cache.has_key(name):
            logger.debug(f"Getting an existing {name} PatternSet")
            return self.get_one(name)

        # Callable[[T], T]
        logger.debug("Creating a new {name} PatternSet")
    ...
    def _before_save(self, pset: PatternSet, data: PatternSetData) -> PatternSet:
        ...
        logger.info(f"Loading {len(pattern_data_list)} patterns for '{pset.name}'")
