...
class TestVerbVerbDetector(unittest.IsolatedAsyncioTestCase):
    async def assertVerbText_(self, value, expected_texts):
        result = await detect_verbs(value)
        count = 0
    ...
    async def test_returns_a_list_of_serializable_objects(self):
        result = await detect_verbs("I ran to the beach.")
    ...
    async def test_true_negative(self):
        with self.assertRaises(TypeError):
            await detect_verbs(100)
            await detect_verbs(True)
            await detect_verbs(["I ran to the beach."])
            await detect_verbs(None)
