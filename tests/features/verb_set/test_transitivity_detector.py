...
class TestVerbTransitivityDetector(unittest.IsolatedAsyncioTestCase):
    async def assertTransitivity_(self, input, expected_transitivity, expected_valency):
        (result_transitivity, result_valency) = await detect_verb_transitivity(input)
    ...
    async def test_returns_a_tuple_of_str_and_int(self):
        match = await detect_verb_transitivity("The dog chased the cat.")
    ...
    async def test_true_positive_impersonal(self):
    ...
    async def test_true_positive_intransitive(self):
        await asyncio.gather(
            # simple
    ...
    async def test_true_positive_transitive(self):
        await asyncio.gather(
            # simple
    ...
    async def test_true_positive_ditransitive_dobj_prep_pobj(self):
        await asyncio.gather(
            # simple
    ...
    async def test_true_positive_ditransitive_dative_det_dobj(self):
        await asyncio.gather(
            # simple
    ...
    async def test_true_negative(self):
        with self.assertRaises(TypeError):
            await detect_verb_transitivity(100)
            await detect_verb_transitivity(True)
            await detect_verb_transitivity(["It rained."])
            await detect_verb_transitivity(None)

        with self.assertRaises(ValueError):
            await detect_verb_transitivity("")
